...
struct ntp_control {
	...
	int cross_check;
	char serv_addr[4];
	int skew_set;  //when time skew >?ms, set time
};
...
#undef FAIL
	...
	//printf("skew=%f %d\n", fabs(skew1-skew2)/2, ntpc->skew_set*1000);
	/* XXX should I do this if debug flag is set? */
	if (ntpc->set_clock||fabs(skew1-skew2)/2>ntpc->skew_set*1000) { /* you'd better be root, or ntpclient will exit here! */
		set_time(&xmttime);
	}
...
static void usage(char *argv0)
{
	...
	"\t[-i interval] [-k skew_set][-l] [-p port] [-q min_delay]"
#ifdef ENABLE_REPLAY
	" [-r]"
#endif
	" [-s] [-t]\n",
...
int main(int argc, char *argv[]) {
	...
	for (;;) {
		c = getopt( argc, argv, "c:" DEBUG_OPTION "f:g:h:i:k:lp:q:" REPLAY_OPTION "st");
		if (c == EOF) break;
		switch (c) {
			case 'c':
